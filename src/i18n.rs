...
pub struct Strings {
    ...
    /// Same prompt, but the capture will add to the row rather than
    /// replace it — worth wording differently, since the two look
    /// identical and only one is about to discard what is there.
    pub press_key_add: &'static str,
    pub press_button_add: &'static str,
...
pub const EN: Strings = Strings {
    ...
    press_key_add: "PRESS KEY TO ADD...",
    press_button_add: "PRESS BUTTON TO ADD...",
    ui_up: "MENU UP",
    ui_down: "MENU DOWN",
    ui_left: "MENU LEFT",
    ui_right: "MENU RIGHT",
    ...
    language: "Language",
    back: "BACK",
    settings_hint: "ENTER: rebind    Right: add another    Left: remove    ESC: back",
    pad_hint: "GAMEPAD: menu buttons are under MENU CONTROLS. An action can hold several buttons at once. The stick always navigates and always moves the piece (up = hard drop); B backs out unless you bind it to something else",
...
pub const JA: Strings = Strings {
    ...
    press_key: "キーを入力...",
    press_button: "ボタンを入力...",
    press_key_add: "追加するキーを入力...",
    press_button_add: "追加するボタンを入力...",
    ui_up: "メニュー上",
    ui_down: "メニュー下",
    ui_left: "メニュー左",
    ui_right: "メニュー右",
    ...
    language: "言語",
    back: "戻る",
    settings_hint: "ENTER: 変更    →: 追加    ←: 削除/調整    ESC: 戻る",
    pad_hint: "パッド: メニュー操作は[メニュー操作]で変更可。1つの操作に複数のボタンを割り当てられる。スティックは常にカーソル移動と操作(上=ハードドロップ)、Bは他に割り当てない限り戻る",
    sec_keys: "- キー設定 -",
